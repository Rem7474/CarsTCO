...
describe('App', () => {
  ...
  it('updates the vehicle label live as the user types', async () => {
    ...
    const summaryTable = screen.getByRole('table', { name: 'Tableau de synthèse' })
  ...
  it('recomputes the total cost when the annual mileage changes', async () => {
    ...
    const table = screen.getByRole('table', { name: 'Tableau de synthèse' })
    ...
    const totalRowAfter = within(screen.getByRole('table', { name: 'Tableau de synthèse' })).getByText('Coût total').closest('tr')!
